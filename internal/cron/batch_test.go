...
func TestUpdateStatsAtomicBatch(t *testing.T) {
	...
	if err := database.Exec(ctx, `create trigger reject_sizes before insert on size_stats
	...
	for _, table := range []string{"hit_counts", "ref_counts", "location_stats", "language_stats", "size_stats"} {
		...
		var count int
		if err := database.Get(ctx, &count, "select sum("+column+") from "+table); err != nil {
			t.Fatal(err)
		}
		if count != 1 {
	...
	if err := database.Exec(ctx, `drop trigger reject_sizes`); err != nil {
		t.Fatal(err)
	}
	if err := cron.UpdateStats(ctx, hits); err != nil {
		t.Fatal(err)
	...
	var count int
	if err := database.Get(ctx, &count, `select sum(total) from hit_counts`); err != nil || count != 2 {
