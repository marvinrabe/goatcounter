...
// jobs formats the wanted job list; %[1]s is today's date.
func jobs(want ...string) []string {
	today := time.Now().Format("2006-01-02")
	for i := range want {
		want[i] = fmt.Sprintf(want[i], today)
	}
	return want
}
...
func TestHistory(t *testing.T) {
	...
	want := jobs(
		"test %[1]s 50ms bgrun_test.go",
		"test %[1]s 50ms bgrun_test.go",
		"test2 %[1]s 50ms bgrun_test.go",
		"test2 %[1]s 50ms bgrun_test.go",
		"test3 %[1]s 50ms bgrun_test.go",
		"test3 %[1]s 50ms bgrun_test.go",
	)
...
func TestRunning(t *testing.T) {
	...
	want := jobs(
		"test %[1]s 0s bgrun_test.go",
		"test %[1]s 0s bgrun_test.go",
		"test2 %[1]s 0s bgrun_test.go",
	)
