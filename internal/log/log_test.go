...
func newText(buf *bytes.Buffer) slog.Handler {
...
func TestContext(t *testing.T) {
	...
	doText := func(ctx context.Context) string {
		buf := new(bytes.Buffer)
		slog.SetDefault(slog.New(newText(buf)))
		do(ctx)
		return re.ReplaceAllString(buf.String(), `$1.go:YYY`)
	}
	...
	t.Run("empty context", func(t *testing.T) {
		...
		t.Run("text", func(t *testing.T) {
			have := doText(ctx)
			want := `
				time=1985-06-18T13:14:15.000Z level=INFO msg=MSG attr=123
				time=1985-06-18T13:14:15.000Z level=INFO msg="MOD: MSG" attr=123
				time=1985-06-18T13:14:15.000Z level=ERROR msg="oh noes" stacktrace="\n\tlog.go:YYY   github.com/marvinrabe/goatcounter/internal/log.(*Logger).Error\n" "some attr"=xxx
				time=1985-06-18T13:14:15.000Z level=ERROR msg="M: oh noes" stacktrace="\n\tlog.go:YYY   github.com/marvinrabe/goatcounter/internal/log.(*Logger).Error\n" "some attr"=xxx
			`
	...
	t.Run("attrs from context", func(t *testing.T) {
		...
		t.Run("text", func(t *testing.T) {
			have := doText(ctx)
			want := `
				time=1985-06-18T13:14:15.000Z level=INFO msg=MSG attr=123 one=1 moar="[xx yy]" one="another one"
				time=1985-06-18T13:14:15.000Z level=INFO msg="MOD: MSG" attr=123 one=1 moar="[xx yy]" one="another one"
...
func TestAttrHTTP(t *testing.T) {
	...
	t.Run("text", func(t *testing.T) {
		...
		have := buf.String()
		want := `
			time=1985-06-18T13:14:15.000Z level=INFO msg=msg attr1=val1 http.verb=GET http.url=http://example.com/path http.host=example.com http.ua="~Z TestBrowser/1.0"
			`
		if d := ztest.Diff(have, want, ztest.DiffNormalizeWhitespace); d != "" {
			t.Error(d)
		}
