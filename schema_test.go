	...
	for _, table := range []struct {
		...
		t.Run(table.name, func(t *testing.T) {
			...
			for _, day := range []string{"2024-02-29", "2026-09-12"} {
				if err := database.Exec(ctx, query, day); err != nil {
...
func TestDashboardQueriesUseTimeIndexes(t *testing.T) {
	...
	if err := database.Exec(ctx, `insert into paths (path_id, site, path) values (1, 'example.com', '/'), (2, 'other.example', '/')`); err != nil {
		t.Fatal(err)
	}
	for _, query := range []string{
		...
		if err := database.Exec(ctx, `with recursive hours(n) as (
	...
	if err := database.Exec(ctx, `analyze`); err != nil {
		t.Fatal(err)
	}
	for _, tt := range []struct {
		...
		t.Run(tt.query, func(t *testing.T) {
			...
			query, isTemplate, err := database.Load(database.MustGetDB(ctx), tt.query)
			if err != nil {
				t.Fatal(err)
			}
			if isTemplate {
				rendered, err := database.Template(query, params)
				if err != nil {
					t.Fatal(err)
				}
			...
			if err := database.Select(ctx, &plan, "explain query plan "+query, params); err != nil {
				t.Fatal(err)
			}
			var found bool
